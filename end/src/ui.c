...
void ui_init(void)
{
    ...
    cbreak(); noecho(); curs_set(0); timeout(0); keypad(stdscr, 1);
    if (has_colors()) {
        start_color();
...
void ui_refresh(void)
{
    ...
    if (last_h != h || last_w != w) {
        /* Destroy old windows and NULL out pointers to prevent double-free */
        if (title_win)  { delwin(title_win);  title_win  = NULL; }
        if (list_win)   { delwin(list_win);   list_win   = NULL; }
        if (detail_win) { delwin(detail_win); detail_win = NULL; }
        if (stats_win)  { delwin(stats_win);  stats_win  = NULL; }
    ...
    if (total_filtered > visible_rows) {
    ...
    /* Draw visible range */
    int drawn = 0;
    int start_idx = list_scroll;

    for (int i = 0; i < pkt_count && drawn < visible_rows; i++) {
        if (!match_filter(packets[i].proto_tag)) continue;
        if (drawn < start_idx) { drawn++; continue; }

        int row = 1 + (drawn - start_idx);
        if (row >= body_h) break;
    ...
    if (pkt_count > 0 && pkt_selected >= 0 && pkt_selected < pkt_count) {
        ...
        case VIEW_TRAFFIC:
            ...
            for (int i = 0; i < nflows && i < 20; i++) {
                ...
                snprintf(sip, sizeof(sip), "%u.%u.%u.%u",
                         (sorted[i].key.sip >> 0) & 0xFF,
                         (sorted[i].key.sip >> 8) & 0xFF,
                         (sorted[i].key.sip >> 16) & 0xFF,
                         (sorted[i].key.sip >> 24) & 0xFF);
                snprintf(dip, sizeof(dip), "%u.%u.%u.%u",
                         (sorted[i].key.dip >> 0) & 0xFF,
                         (sorted[i].key.dip >> 8) & 0xFF,
                         (sorted[i].key.dip >> 16) & 0xFF,
                         (sorted[i].key.dip >> 24) & 0xFF);
        ...
        case VIEW_APPINFO:
            /* DNS domains + TLS SNI + HTTP methods */
            {
                int row = 1;
                char domains[50][256];
                int nd = ui_get_dns_domains(domains, 50);
                if (nd > 0) {
                ...
                char snis[50][256];
                int ns = ui_get_tls_snis(snis, 50);
                if (ns > 0) {
                ...
                int nm = 0;
                pthread_mutex_lock(&ui_mutex);
                for (int i = 0; i < pkt_count && nm < 20; i++) {
                    if (packets[i].http_method[0] != '\0') {
                        int dup = 0;
                        for (int j = 0; j < nm; j++) {
    ...
    mvwprintw(stats_win, 0, 1,
              "Sniffer | iface:%s filter:%s uptime:%s total:%lu",
              g_capture_dev, g_capture_filter, dur, total_packets);
    mvwprintw(stats_win, 1, 1,
              "View: %s | TCP:%d UDP:%d ICMP:%d DNS:%d HTTP:%d ActiveFlows:%d | %s%s",
              view_names[g_current_view],
              g_stat.pkt_tcp, g_stat.pkt_udp, g_stat.pkt_icmp,
              g_stat.pkt_dns, g_stat.pkt_http,
              tcp_flow_active_count(),
              pause_text, g_filter_hint);
    mvwprintw(stats_win, 2, 1,
              "Hotkeys: Tab=<->View  Up/Dn=List  PgUp/PgDn=Scroll  Space=Pause  C=Clear  Q=Quit");
...
void ui_handle_input(void)
{
    ...
    /* Tab / arrows: switch views */
    if (ch == KEY_BTAB || ch == 258) {       /* Shift+Tab → prev */
        g_current_view = (g_current_view == 0) ? VIEW_COUNT - 1 : g_current_view - 1;
        detail_scroll = 0;
        return;
    }
    if (ch == 9 || ch == 258) {                   /* Tab → next */
    ...
    if (ch == '7') { g_current_view = VIEW_PACKETS; detail_scroll = 0; return; }
    if (ch == '8') { g_current_view = VIEW_TRAFFIC; detail_scroll = 0; return; }
    if (ch == '9') { g_current_view = VIEW_APPINFO; detail_scroll = 0; return; }
