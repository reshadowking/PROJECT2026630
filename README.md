...
- **分桶锁**：256 个独立 mutex (TCP 流 / 流量统计各一套)，大幅降低竞争
- **大内核缓冲**：256MB SO_RCVBUF + 512MB pcap user buffer
- **immediate mode**：减少延迟
...
| 文件 | 说明 |
|------|------|
| `http_pairs.log` | HTTP 请求-响应配对日志 (追加模式) |
| `*.pcap` | 通过 `-w` 参数指定的抓包文件 |
...
### 在线性能测试 (需 root)
```bash
sudo bash perf_test.sh 50000    # 5 万包性能测试
sudo bash run_all_tests.sh --live  # 完整测试 (含 Live 抓包)
```

### HTTP Parser 单元测试
```bash
gcc -Wall -Wextra -g -I./include -o test_http_parser test_http_parser.c src/http_parser.c src/logger.c -lpthread
./test_http_parser
```

### 验证要点
1. 编译零警告
2. ICMP / DNS / HTTP / TLS 四种协议均有解析输出
3. HTTP 流重组提取 ≥ 50 条以上完整请求
4. 高流量场景丢包率 < 1%
...
├── perf_test.sh        # 性能测试脚本
├── gen_test_pcap.py    # 测试 PCAP 生成脚本
└── test_http_parser.c  # HTTP 解析器单元测试
```
