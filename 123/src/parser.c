...
void parse_http(const u_char *payload, int len, char *method_out, int method_sz)
{
    ...
    /* Recognise common HTTP methods (all with trailing space for exact match) */
    ...
    for (i = 0; i < n_methods; i++) {
        ...
        if (mlen > 0 && mlen <= len &&
            !strncmp((const char *)payload, methods[i], mlen)) {
            stat_inc_http();
            if (method_out && method_sz > 0) {
                int copy_len = mlen - 1; /* strip trailing space */
                if (copy_len >= method_sz) copy_len = method_sz - 1;
                strncpy(method_out, methods[i], copy_len);
                method_out[copy_len] = '\0';
            }
            return;
        }
    }

    /* Also detect HTTP response lines (HTTP/1.x) */
    if (len >= 7 && !strncmp((const char *)payload, "HTTP/1.", 7)) {
        stat_inc_http();
        if (method_out && method_sz > 0) {
            strncpy(method_out, "RESP", method_sz - 1);
               ...
               char *tls_sni, int sni_sz, char *http_method, int method_sz)
{
    ...
    if (dport == PORT_HTTP || sport == PORT_HTTP ||
        ...
        tcp_flow_add(sip, dip, sport, dport, seq, tcp->flags, pay, pay_len);
    }
...
void parse_ipv4(const u_char *data, int len)
{
    ...
    int effective_len = (total_len < len) ? total_len : len;
    const u_char *trans_ptr = data + ihl;
    int trans_len = effective_len - ihl;
